...
document.getElementById('addup_modes').innerHTML=allmodes_html;
}
...
function loading()
{
  ...
  setTimeout( function() {
   stoploading(console.log('end')+nothing);
},100);
var nothing=console.log(execute_mode());
...
startloading();
var key1=document.getElementById('keyword4search').value.split('');
       var num=0;
switch(node_choosed)
{
case 'autoblank':
...
var node_choosed=myselect.options[index].value;
  list=getURLString('./modules/phone/phone/phone_num_info.php?showinfo='+node_choosed).split('#');
  let location='';
  for(part in list)
  {
    if(list[part]!='')
...
{
  list=getURLString('./modules/phone/info/phone_num_info.php?showinfo=provience').split('#');
  let location='';
...
url='./modules/phone/phone/phone_num_info.php?'+url_add;
//console.log('url: '+url);
